#!/usr/bin/env python3
"""Run QASMTrans (Python API) on VQE/UCCSD circuits and collect metrics."""
...
def run_qasmtrans(circuit_qasm: str, device_json: Path) -> Tuple[float, float, Tuple[str, str, str]]:
    ...
    out_qasm = ""
    if isinstance(res.output_qasm, str) and res.output_qasm:
        out_qasm = res.output_qasm
    elif isinstance(res.output_qasm_path, str) and res.output_qasm_path:
        try:
    ...
    if out_qasm:
        try:
            qc = QuantumCircuit.from_qasm_str(out_qasm)
            one_val, two_val = count_gate_types(qc)
            one_qubit = str(one_val)
            two_qubit = str(two_val)
            depth = str(qc.depth())
        except Exception:
            one_qubit = two_qubit = depth = ""
