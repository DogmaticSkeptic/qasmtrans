#!/usr/bin/env python3
"""
Run Mapomatic benchmarks and generate both fidelity and timing plots in one go.

This replaces:
  - scripts/run_mapomatic_bench.sh
  - scripts/plot_mapomatic_fidelity.py
  - scripts/plot_mapomatic_timing.py

It:
  1) Runs qasmtrans for each circuit under the requested Mapomatic modes.
  2) Runs NWQ-Sim on the emitted subchip to extract fidelity.
  3) Writes a CSV (circuit, mode, total_ms, mapomatic_ms, fidelity).
  4) Generates two PDFs: fidelity bar chart and timing percentage plot.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--input",
        action="append",
        help="QASM circuit file(s). If omitted, defaults to scripts/tmp/{vqe8,qpe9,sat7,shor7}.qasm",
    )
    parser.add_argument(
        "--modes",
    ...
    parser.add_argument(
        "--output_csv",
        default=None,
        help="Where to write the benchmark CSV (default: test/benchmarking/mapomatic_benchmarks.csv).",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
        help="Directory to store transpiled outputs (default: test/benchmarking/output_qasm_file).",
    )
    parser.add_argument(
        "--mapomatic_limit",
        type=int,
    ...
    parser.add_argument(
        "--fidelity_pdf",
        default=None,
        help="Path for fidelity plot PDF (default: test/benchmarking/mapomatic_fidelity.pdf).",
    )
    parser.add_argument(
        "--timing_pdf",
        default=None,
        help="Path for timing plot PDF (default: test/benchmarking/mapomatic_timing.pdf).",
    )
    ...
    output_csv = Path(args.output_csv).resolve() if args.output_csv else (SCRIPT_DIR / "mapomatic_benchmarks.csv")
    output_dir = Path(args.output_dir).resolve() if args.output_dir else (SCRIPT_DIR / "output_qasm_file")
    fidelity_pdf = Path(args.fidelity_pdf).resolve() if args.fidelity_pdf else (SCRIPT_DIR / "mapomatic_fidelity.pdf")
    timing_pdf = Path(args.timing_pdf).resolve() if args.timing_pdf else (SCRIPT_DIR / "mapomatic_timing.pdf")
    ...
    if not args.skip_plots:
        plot_fidelity(output_csv, fidelity_pdf)
        plot_timing(output_csv, timing_pdf)
        print(f"Wrote plots to {fidelity_pdf} and {timing_pdf}")
