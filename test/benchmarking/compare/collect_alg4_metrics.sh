#!/usr/bin/env bash
# Collect fidelity/latency metrics for 4-qubit benchmark circuits (qft4/shor4/adder4/w4/bv4/ghz4).
# Requires: jq, QASMTrans build, and Python deps for merge/sim scripts.
...
if [[ $DO_PLOT -eq 1 ]]; then
    echo "Generating plots via test/benchmarking/plots/plot_alg4_metrics.py"
    python "${REPO_DIR}/test/benchmarking/plots/plot_alg4_metrics.py"
fi
