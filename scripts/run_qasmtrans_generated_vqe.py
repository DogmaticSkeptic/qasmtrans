...
def run_qasmtrans(
    ...
    for attempt in range(1, attempts + 1):
        ...
        if "[metrics]" not in stdout:
            print("  metrics missing, retrying")
            continue
        ...
        metrics_line = next(
            (line for line in stdout.splitlines() if line.startswith("[metrics]")),
            None,
        )
        parts = dict(
            item.split("=") for item in metrics_line.split() if "=" in item
        )
        one_qubit = int(parts.get("one_qubit_gates", 0))
        two_qubit = int(parts.get("two_qubit_gates", 0))
