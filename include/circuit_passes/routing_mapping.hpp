...
vector<IdxType> pick_one_movement(vector<IdxType> &mapping,
  ...
  for (size_t pair_idx = 0; pair_idx < possible_pairs.size(); ++pair_idx) {
    ...
    score[pair_idx] = heuristic(temp_l2p_mapping, current_layer, future_layer,
                                distance_mat, circuit);
  }
...
IdxType one_round_optimization(
  ...
  while (executed_gates_num < gate_num) {
    set<IdxType> execute_gates_idx =
        find_executable_gates(mapping, current_layer, circuit, distance_mat);
...
void Routing(shared_ptr<Circuit> circuit, shared_ptr<Chip> chip,
  ...
  IdxType swap_num = one_round_optimization(
  ...
  swap_num = one_round_optimization(
      initial_mapping, cx_gates, chip->distance_mat, gate_info, chip,
      reverse_gate_qubit, return_circuit, debug_level);
