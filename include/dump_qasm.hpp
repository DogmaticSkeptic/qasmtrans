...
std::string dumpQASM(std::shared_ptr<QASMTrans::Circuit> circuit, const char *filename, const std::string &requested_output_path, IdxType debug_level, IdxType mode)
{
    ...
    for (auto &creg : cregs)
    {
        for (auto &qubit : creg.second.qubit_indices)
        {
            ...
            {
                throw std::runtime_error("Error: measurement mapping size is smaller than creg width.");
            }
            IdxType mapped = circuit->initial_mapping[creg_index];
            if (mapped >= 0)
            {
                qasm_file << "measure q[" << mapped << "] -> " << toLowerCase(creg.first) << "[" << creg_index << "];\n";
            }
            ++creg_index;
        }
    }
