...
namespace
{
    ...
    {
        // Map creg bits to the final physical order.
...
struct CliConfig
{
    ...
    bool enable_2q_cancel = false;
    bool enable_commute_2q = false;
    bool enable_2q_synth = false;
    bool enable_sabre_layout = false;
...
bool parse_cli(int argc, char **argv, CliConfig &config, int &exit_code)
{
    ...
    if (cmdOptionExists(argv, argv + argc, "--optimize-1q"))
    {
        config.enable_1q_opt = true;
    }
    if (cmdOptionExists(argv, argv + argc, "--optimize-2q-cancel"))
    {
        config.enable_2q_cancel = true;
    }
    if (cmdOptionExists(argv, argv + argc, "--optimize-commute-2q"))
    {
        config.enable_commute_2q = true;
    }
    if (cmdOptionExists(argv, argv + argc, "--optimize-2q-synth"))
    {
    ...
    while (argi < argc)
    {
        ...
        if (current == "--optimize-2q-cancel")
        {
            config.enable_2q_cancel = true;
            ++argi;
            continue;
        }
        if (current == "--optimize-commute-2q")
        {
            config.enable_commute_2q = true;
            ++argi;
            continue;
        }
        if (current == "--optimize-2q-synth")
        {
            config.enable_2q_synth = true;
            ++argi;
            continue;
                                      ...
                                      IdxType total_requested_qubits)
{
    ...
    for (std::size_t ci = 0; ci < circuits.size(); ++ci)
    {
        ...
        transpiler(circuit,
                   ...
                   config.enable_2q_cancel,
                   config.enable_commute_2q,
                   config.enable_2q_synth,
                   ...
                   config.seed >= 0 ? std::optional<uint64_t>(static_cast<uint64_t>(config.seed)) : std::nullopt);

        artifacts.routing_swap_count += circuit->get_routing_swap_count();

        // Capture per-circuit artifacts before global remap.
        ...
        // Translate local physical indices to device-global.
        ...
        // Prefix creg names for merged QASM.
...
void export_subchips(const std::vector<SubchipArtifact> &subchip_artifacts,
                     ...
                     const CliConfig &config)
{
    // Create subchip output directory.
