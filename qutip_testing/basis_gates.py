...
def synth_su2_via_Rz_Rx90(alpha, beta, gamma):
...
def step_fidelities_matched(blocks, dt_ns=0.25, sigma_frac=0.2):
    ...
    for kind, args in blocks:
        if kind == "rz":
            ...
            # No change to U_cum: virtual Rz is a frame update, not a physical unitary on the state
        elif kind == "rx_pi2":
            t, Iq, Qq, Jq, _, _ = run_rx(np.pi / 2.0, args["duration_ns"], 0,
                                         dt_ns=dt_ns, sigma_frac=sigma_frac, phase=phase[0])
            t_blk = t if t_all.size == 0 else t_all[-1] + dt + t
            ...
            for q in [0, 1]:
                ...
                Q_all[q] = Qq[q] if I_all[q].size == 0 else np.concatenate([Q_all[q], Qq[q]])
            ...
            t, Iq, Qq, Jq, _, _ = run_rx(np.pi, args["duration_ns"], 0,
                                         dt_ns=dt_ns, sigma_frac=sigma_frac, phase=phase[0])
            t_blk = t if t_all.size == 0 else t_all[-1] + dt + t
            t_all = t_blk if t_all.size == 0 else np.concatenate([t_all, t_blk])
            for q in [0, 1]:
...
def main():
    ...
    a2, b2, c2 = synth_su2_via_Rz_Rx90(alpha, beta, gamma)
